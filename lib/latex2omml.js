...
function parseTerm(tokens, pos) {
  ...
  if (tok.t === TOK_CMD) {
    ...
    // Matrix / aligned environments: \begin{pmatrix} ... \end{pmatrix}
    if (cmd === 'begin') {
      var env = parseBraceText(tokens, pos);
      var isMat = /^(p|b|B|v|V)?matrix$/.test(env);
      if (isMat) {
        var bL = '('; var bR = ')';
        if (env === 'bmatrix') { bL = '['; bR = ']'; }
        else if (env === 'Bmatrix') { bL = '{'; bR = '}'; }
        else if (env === 'vmatrix') { bL = '|'; bR = '|'; }
        else if (env === 'Vmatrix') { bL = '\u2016'; bR = '\u2016'; }
        else if (env === 'matrix')  { bL = ''; bR = ''; }
        var rows = [], curRow = [];
        while (pos.i < tokens.length && tokens[pos.i].t !== TOK_EOF) {
          if (tokens[pos.i].t === TOK_CMD && tokens[pos.i].v === 'end') {
            var saveI = pos.i; pos.i++;
            var endEnv = parseBraceText(tokens, pos);
            if (endEnv === env) { if (curRow.length > 0) rows.push(curRow); break; }
            pos.i = saveI;
          }
          if (tokens[pos.i].t === TOK_TEXT) {
            if (tokens[pos.i].v === '\\') {
              pos.i++;
              // Handle \\[dim] optional spacing
              if (tokens[pos.i] && tokens[pos.i].t === TOK_LBRACE && tokens[pos.i + 1] && tokens[pos.i + 1].t === TOK_RBRACE) {
                pos.i += 2;
              }
            ...
            if (tokens[pos.i].v === '&') {
              pos.i++;
              curRow.push([new docx.MathRun('')]); continue;
            }
          }
          var cell = [];
          while (pos.i < tokens.length && tokens[pos.i].t !== TOK_EOF) {
            if (tokens[pos.i].t === TOK_TEXT && (tokens[pos.i].v === '&' || tokens[pos.i].v === '\\')) break;
            if (tokens[pos.i].t === TOK_CMD && tokens[pos.i].v === 'end') break;
            var term = parseTerm(tokens, pos);
            if (term) cell.push.apply(cell, Array.isArray(term) ? term : [term]);
            else break;
          }
          curRow.push(cell);
        }
        var mruns = [];
        if (bL) mruns.push(new docx.MathRun(bL));
        for (var ri = 0; ri < rows.length; ri++) {
          if (ri > 0) mruns.push(new docx.MathRun('\n'));
          var r = rows[ri];
          for (var ci = 0; ci < r.length; ci++) {
            if (ci > 0) mruns.push(new docx.MathRun('  '));
            mruns.push.apply(mruns, r[ci]);
          }
        }
        if (bR) mruns.push(new docx.MathRun(bR));
        return mruns;
      }
      // Unknown env: skip to matching \end
      var depth = 1;
      while (pos.i < tokens.length && depth > 0) {
        if (tokens[pos.i].t === TOK_CMD && tokens[pos.i].v === 'begin') {
          pos.i++; parseBraceText(tokens, pos); depth++;
        } else if (tokens[pos.i].t === TOK_CMD && tokens[pos.i].v === 'end') {
          pos.i++; parseBraceText(tokens, pos); depth--;
        } else { pos.i++; }
      }
