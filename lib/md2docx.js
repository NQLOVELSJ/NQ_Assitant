...
function tokensToRuns(tokens) {
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    switch (t.type) {
      case 'text':
        pushTextWithMath(runs, t.text || '');
        break;
      case 'strong':
        var strongRuns = tokensToRuns(t.tokens);
        strongRuns.forEach(function(r) { r.bold = true; });
        runs.push.apply(runs, strongRuns);
        break;
      case 'em':
        var emRuns = tokensToRuns(t.tokens);
        emRuns.forEach(function(r) { r.italics = true; });
        runs.push.apply(runs, emRuns);
        break;
      case 'del':
        var delRuns = tokensToRuns(t.tokens);
        delRuns.forEach(function(r) { r.strike = true; });
        runs.push.apply(runs, delRuns);
        break;
      case 'codespan':
      ...
      case 'link':
        ...
        runs.push(new docx.ExternalHyperlink({
          children: [new docx.TextRun({ text: linkText, style: 'Hyperlink', color: '0563C1', underline: { type: docx.UnderlineType.SINGLE } })],
          link: t.href || ''
        }));
        break;
      case 'image':
      ...
      case 'html':
        break;
      default:
        if (t.text) pushTextWithMath(runs, t.text);
        else if (t.tokens) runs.push.apply(runs, tokensToRuns(t.tokens));
    }
  }
...
function pushTextWithMath(runs, text) {
  if (!text) return;
  var pos = 0;
  ...
  while (pos < tlen) {
    ...
    if (remaining.indexOf('$$') === 0) {
      ...
      runs.push(tr(remaining.substring(0, 2)));
      pos += 2;
      continue;
    }
    ...
    if (imMatch) {
      ...
      if (hasMath) {
        if (imIdx > 0) runs.push(tr(remaining.substring(0, imIdx)));
        try {
          if (typeof latex2docx === 'function') {
