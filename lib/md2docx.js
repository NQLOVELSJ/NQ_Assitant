...
var _mathBlocks = []; // { latex: '...' }

function replaceMathSentinels(md) {
  _mathBlocks = [];
  return md.replace(/\$\$([\s\S]*?)\$\$/g, function(m, inner) {
    var id = _mathBlocks.length;
    _mathBlocks.push({ latex: inner.trim() });
    return '\n<!--NQMATH_' + id + '-->\n';
  });
}

function restoreMathParagraph(latex) {
    ...
    if (typeof latex2docx === 'function') {
      ...
      if (mc && mc.length > 0) {
        return new docx.Paragraph({
          ...
          children: [new docx.Math({ children: mc })]
        });
      }
    }
  } catch(e) {}
  // Fallback: display as monospace math text
  return new docx.Paragraph({
    spacing: { before: 80, after: 80 },
    children: [new docx.TextRun({ text: latex, font: 'Consolas', size: 20, italics: true })]
  });
}

function isMathSentinel(paraText) {
  return /^<!--NQMATH_\d+-->$/.test(paraText);
}
...
function md2docxChildren(markdown, opts) {
  ...
  // Protect $$...$$ blocks from marked.lexer() splitting
  var md = replaceMathSentinels(markdown);
...
function tokensToDocx(tokens) {
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    // Check for math sentinel
    if (paraText && isMathSentinel(paraText)) {
      var mid = parseInt(paraText.match(/\d+/)[0], 10);
      if (_mathBlocks[mid]) {
        var mathPara = restoreMathParagraph(_mathBlocks[mid].latex);
  ...
  listToken.items.forEach(function(item) {
    ...
    if (item.tokens) {
      for (var ti = 0; ti < item.tokens.length; ti++) {
        ...
        else if (st.type === 'text' || st.type === 'paragraph' || st.type === 'space') { /* skip */ }
...
// ============ Math detection & OMML conversion ============
...
function parseHighlightedSpans(html) {
  // Simple regex parser for <span class="...">text</span>
  var tokens = [];
    ...
    if (tokens.length > 0) {
      ...
      tokens.forEach(function(tok) {
        ...
        for (var pi = 0; pi < parts.length; pi++) {
          ...
          if (parts[pi]) {
            currentLine.push({ text: parts[pi], color: tok.color });
          }
        }
      });
