 ...
 * Supports block math rendering as images via KaTeX + html2canvas
 * Depends: docx.bundle.js, marked (global), latex2omml.js, hljs, katex, html2canvas
 */
'use strict';
...
// ============ Math image cache ============
var _mathImages = {}; // cacheId -> { blob: Blob, width: number, height: number }

function renderMathToImage(latex, displayMode) {
  return new Promise(function(resolve) {
    var div = document.createElement('div');
    div.style.cssText = 'position:fixed;left:-9999px;top:0;background:#fff;padding:10px;';
    try {
      div.innerHTML = katex.renderToString(latex, { displayMode: !!displayMode, throwOnError: false, strict: false });
    } catch(e) {
    ...
    setTimeout(function() {
      try {
        html2canvas(div, { backgroundColor: '#ffffff', scale: 2, logging: false }).then(function(canvas) {
...
// ============ Math sentinel: protect $$...$$ from marked.lexer(), replace with images ============
var _mathSentinelId = 0;
var _sentinelLaTeX = {}; // id -> { latex, displayMode }
...
function preProcessMath(md) {
  ...
  return md.replace(/\$\$([\s\S]*?)\$\$/g, function(m, inner) {
    ...
    _sentinelLaTeX[id] = { latex: inner.trim(), displayMode: !!m };
    return '\n<!--MATH_' + id + '-->\n';
  });
}
...
function md2docxChildren(markdown, opts) {
  ...
  // Pre-render math images for all sentinels
  var renderPromises = [];
  for (var id in _sentinelLaTeX) {
    if (_sentinelLaTeX.hasOwnProperty(id)) {
      renderPromises.push(
        renderMathToImage(_sentinelLaTeX[id].latex, _sentinelLaTeX[id].displayMode)
        .then(function(result) {
          if (result) _mathImages[id] = result;
        })
      );
    }
  }
...
function tokensToDocx(tokens) {
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    // Check for math sentinel → replace with image
    if (paraText && isMathSentinel(paraText)) {
      var mid = getMathSentinelId(paraText);
      var img = _mathImages[mid];
      if (img && img.blob) {
        children.push(createMathImageParagraph(img));
        continue;
      }
      // Fallback: render LaTeX as monospace text
      var fallLatex = _sentinelLaTeX[mid] ? _sentinelLaTeX[mid].latex : paraText;
      children.push(new docx.Paragraph({
        spacing: { before: 80, after: 80 },
        children: [new docx.TextRun({ text: fallLatex, font: 'Consolas', size: 20, italics: true })]
      }));
      continue;
    }
...
function createMathImageParagraph(img) {
  var scale = 2;
  var w = Math.round(img.width * 9525 / scale);
  var h = Math.round(img.height * 9525 / scale);
...
function pushTextWithMath(runs, text, _fmt) {
  ...
  // Block math $$...$$ should be handled via sentinels by this point.
  // If any remain, treat as plain text.
  ...
  while (pos < tlen) {
    ...
    // $$ sentinel check (should not normally reach here)
    if (remaining.indexOf('$$') !== -1) {
      var bs = remaining.indexOf('$$');
      if (bs > 0) runs.push(t(remaining.substring(0, bs)));
      runs.push(t('$$'));
      pos += bs + 2;
      continue;
    }
    ...
    if (bmMatch && bmMatch.index === 0) {
      ...
      try {
        if (typeof latex2docx === 'function') {
          var mc2 = latex2docx(bm);
          if (mc2 && mc2.length > 0) { runs.push(new docx.Math({ children: mc2 })); pos += bmMatch[0].length; continue; }
        }
      } catch(e) {}
    }
